...
5. Load repeatable Masvingo development data with `npm run prisma:seed`.
...
The seed is idempotent and creates three Masvingo merchants, six products, and development customer/admin/driver users. The web checkout uses `POST /api/v1/users/upsert` to create a customer from a phone number, then sends the selected basket to `POST /api/v1/orders`. Replace the development seed users and role-header placeholders with real authentication before production launch.
